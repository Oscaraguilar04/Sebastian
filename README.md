...
For GitHub Pages, build with a Pages workflow using Node.js 22.12+ and upload `dist/` as the Pages artifact. `vite.config.ts` uses `base: "./"`, so generated assets also work under a repository subdirectory. Keep asset references compatible with that base. The website uses section anchors instead of client-side routes, so a catch-all rewrite is unnecessary.

After choosing the final public URL, add an accurate canonical link and `og:url` to `index.html`, and replace the Open Graph and Twitter image paths with the absolute public URL of `social-preview.jpg`. The relative preview path is portable, but some sharing crawlers require an absolute URL. No invented domain or real-business schema is included.
